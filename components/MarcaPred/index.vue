<template>
        <div class="d-none d-md-block">
          <v-img
            src="/predwhitedesf2.webp"
            width="600"
            style="position: absolute; top: 800px; left: 40%; height: 200px; z-index: 100;"
            alt="Descrição da imagem 1"
          ></v-img>
          <v-img
            src="/predreddesf.webp"
            width="600"
            style="position: absolute; top: 1810px; right: -1%; height: 200px;"
            alt="Descrição da imagem 2"
          ></v-img>
          <NuxtImg src="/predwhitedesf.webp"  cover
          class="d-none d-lg-block w-75"  format="webp" loading="lazy" 
          style="position: absolute; top: 3660px; left: 5%; z-index: 14;" alt="Descrição da imagem 3"/>
          <NuxtImg src="/predwhite.webp" 
          width="100" height="500" class="d-none d-lg-block " format="webp" loading="lazy" 
          style="position: absolute; top: 2250px; left: -2.7%; z-index: 1;"
          alt="Descrição da imagem 4"/>
          <NuxtImg src="/predwhite.webp" 
          width="100" height="500" class="d-none d-lg-block " format="webp" loading="lazy" 
          style="position: absolute; top: 2550px; right: -4.1%; z-index: 1;"
          alt="Descrição da imagem 5"/>
          <NuxtImg src="/predwhite.webp" 
          width="100" height="500" class="d-none d-lg-block " format="webp" loading="lazy" 
          style="position: absolute; top: 3000px; left: -2.9%; z-index: 1;"
          alt="Descrição da imagem 6"/>
          <NuxtImg src="/predblue.webp" 
          width="400" height="1220" class="d-none d-lg-block " format="webp" loading="lazy" 
          style="position: absolute; top: 4150px; right: -7%; z-index: 1;"
          alt="Descrição da imagem 7"/>
        </div>
</template>
      